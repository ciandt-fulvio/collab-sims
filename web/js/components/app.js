...
export function simsApp() {
  return {
    ...
    // Fetch runtime information for this session
    async fetchRuntimeInfo() {
      if (!this.sessionId) return;

      this.isLoadingRuntime = true;
      this.runtimeError = null;

      try {
        // Fetch all runtimes for this session (new multi-runtime API)
        const response = await fetch(`${this.api.baseURL}/api/runtimes/by-session/${this.sessionId}`);
        if (!response.ok) {
        ...
        if (!data.runtimes || data.runtimes.length === 0) {
          // No runtimes associated with this session yet
          this.runtimes = [];
          this.runtimeInfo = null;
          this.selectedRuntimeId = null;
          this.isLoadingRuntime = false;
          return;
        }

        // Store all runtimes
        this.runtimes = data.runtimes;
        console.log(`✅ Loaded ${this.runtimes.length} runtime(s):`, this.runtimes);

        // Don't auto-select - all start collapsed
      } catch (error) {
    ...
    // Toggle runtime expansion (collapsible)
    selectRuntime(runtimeId) {
      // Toggle expansion for this runtime
      if (this.expandedRuntimeIds.has(runtimeId)) {
        this.expandedRuntimeIds.delete(runtimeId);
        // If this was the current runtime showing ports, clear them
        if (this.currentExpandedRuntimeId === runtimeId) {
          this.currentExpandedRuntimeId = null;
        }
      } else {
        this.expandedRuntimeIds.add(runtimeId);
        this.currentExpandedRuntimeId = runtimeId;
        this.runtimeInfo = this.runtimes.find(r => r.runtime_id === runtimeId);
        console.log('Expanded runtime:', this.runtimeInfo);

        // Initialize ports map for this runtime if not exists
        if (!this.runtimePorts.has(runtimeId)) {
          this.runtimePorts.set(runtimeId, { detected: [], exposed: [] });
        }

        // Fetch ports for this runtime
        if (this.runtimeInfo) {
          this.fetchPorts(runtimeId);
        }
      }
    },

    // Check if a runtime is expanded
    isRuntimeExpanded(runtimeId) {
      return this.expandedRuntimeIds.has(runtimeId);
    },

    // Get ports for a specific runtime
    getDetectedPorts(runtimeId) {
      return this.runtimePorts.get(runtimeId)?.detected || [];
    },

    getExposedPorts(runtimeId) {
      return this.runtimePorts.get(runtimeId)?.exposed || [];
    },

    // Fetch detected and exposed ports for a runtime
    async fetchPorts(runtimeId = null) {
      const runtime_id = runtimeId || this.currentExpandedRuntimeId;
      if (!runtime_id) return;

      this.isLoadingPorts.set(runtime_id, true);

      try {
        // Fetch detected ports
        const portsResponse = await fetch(`${this.api.baseURL}/api/runtimes/${runtime_id}/ports`);
        let detectedPorts = [];
        if (portsResponse.ok) {
          const portsData = await portsResponse.json();
          detectedPorts = portsData.ports || [];
        }

        // Fetch exposed ports
        const exposedResponse = await fetch(`${this.api.baseURL}/api/runtimes/${runtime_id}/exposed-ports`);
        let exposedPorts = [];
        if (exposedResponse.ok) {
          const exposedData = await exposedResponse.json();
          exposedPorts = exposedData.exposed_ports || [];
        }

        // Store ports for this specific runtime
        this.runtimePorts.set(runtime_id, {
          detected: detectedPorts,
          exposed: exposedPorts
        });

        console.log(`✅ Ports loaded for ${runtime_id}:`, { detected: detectedPorts.length, exposed: exposedPorts.length });
      } catch (error) {
        console.error('🔴 Failed to fetch ports:', error);
      } finally {
        this.isLoadingPorts.set(runtime_id, false);
      }
    },

    // Check if a port is already exposed for a runtime
    isPortExposed(port, runtimeId = null) {
      const runtime_id = runtimeId || this.currentExpandedRuntimeId;
      return this.getExposedPorts(runtime_id).some(ep => ep.container_port === port);
    },

    // Get exposed port info by container port
    getExposedPortInfo(containerPort, runtimeId = null) {
      const runtime_id = runtimeId || this.currentExpandedRuntimeId;
      return this.getExposedPorts(runtime_id).find(ep => ep.container_port === containerPort);
    },

    // Show expose modal for a port
    showExposePortModal(port, runtimeId = null) {
      console.log('🔵 Opening expose modal for port:', port, 'runtime:', runtimeId);
      this.portToExpose = port;
      this.exposeConfig = { proxy_path: '/', target_path: '/', external_port: null };
      // Set runtimeInfo for the expose action
      if (runtimeId) {
        this.runtimeInfo = this.runtimes.find(r => r.runtime_id === runtimeId);
        console.log('📍 Set runtimeInfo:', this.runtimeInfo);
      }
      this.showExposeModal = true;
      console.log('✅ showExposeModal set to true');
    },

    // Expose a port
    async exposePort() {
      console.log('🔵 exposePort called, runtimeInfo:', this.runtimeInfo, 'portToExpose:', this.portToExpose);
      if (!this.runtimeInfo || !this.portToExpose) {
        console.error('❌ exposePort failed: missing runtimeInfo or portToExpose');
        return;
      }

      try {
        console.log('📤 Sending expose request for port:', this.portToExpose);
        const response = await fetch(`${this.api.baseURL}/api/runtimes/${this.runtimeInfo.runtime_id}/expose-port`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            container_port: this.portToExpose,
            proxy_path: this.exposeConfig.proxy_path || '/',
            target_path: this.exposeConfig.target_path || '/',
            external_port: this.exposeConfig.external_port || null
          })
        });

        if (!response.ok) {
          const error = await response.json();
          throw new Error(error.detail || 'Failed to expose port');
        }

        const result = await response.json();
        console.log('✅ Port exposed:', result);

        // Close modal and refresh ports for this runtime
        this.showExposeModal = false;
        this.portToExpose = null;
        await this.fetchPorts(this.runtimeInfo.runtime_id);
      } catch (error) {
        console.error('🔴 Failed to expose port:', error);
        alert(`Failed to expose port: ${error.message}`);
      }
    },

    // Unexpose a port
    async unexposePort(port, runtimeId = null) {
      const runtime_id = runtimeId || this.currentExpandedRuntimeId;
      if (!runtime_id || !port) {
        console.error('❌ unexposePort failed: missing runtime_id or port');
        return;
      }

      try {
        console.log('📤 Sending unexpose request for port:', port);
        const response = await fetch(`${this.api.baseURL}/api/runtimes/${runtime_id}/unexpose-port`, {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            container_port: port
        ...
        if (!response.ok) {
          const error = await response.json();
          throw new Error(error.detail || 'Failed to unexpose port');
        }

        console.log('✅ Port unexposed');

        // Refresh ports for this runtime
        await this.fetchPorts(runtime_id);
      } catch (error) {
        console.error('🔴 Failed to unexpose port:', error);
        alert(`Failed to unexpose port: ${error.message}`);
      }
    },
